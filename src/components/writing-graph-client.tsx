            ...
            {graphData.map((node) => {
                ...
                return (
                  <Link href={`/writing/${node.slug}`} key={node.slug}>
                    ...
                    >
                      {/* Tooltip - compact with stats */}
                      <div
                        className={`absolute w-max max-w-[200px] bg-[#0a0a0a] border border-[#333] px-2.5 py-2 pointer-events-none transition-all duration-200 shadow-lg
                      ...
                      >
                        <p className="text-[10px] font-medium text-white leading-tight">{node.title}</p>
                        <p className="text-[8px] text-[#555] mt-0.5 mb-1.5">{node.date}</p>
                        {node.attributes && (
                          <div className="grid grid-cols-3 gap-x-2 gap-y-1">
                            {Object.entries(node.attributes).slice(0, 6).map(([key, val]) => (
                              <div key={key} className="flex flex-col text-[7px] text-[#666]">
                                <span className="uppercase opacity-60">{key.slice(0,4)}</span>
                                <div className="w-full h-0.5 bg-[#222] rounded-full overflow-hidden mt-0.5">
                                  <div className="h-full bg-[#555]" style={{ width: `${val * 100}%` }} />
                                </div>
                              </div>
                            ))}
                          </div>
